...
//     private static Vision visionSub = Vision.getInstance();
    ...
    private final JoystickButton coralHeightButton = new JoystickButton(operator, ControllerConstants.b_X);
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand("L4 Place", new ArmMagic(armSub, -58).withTimeout(0.4));
    ...
    private void configureBindings() {
        ...
        slowButton.whileTrue(
                // Drivetrain will execute this command periodically
                drivetrain.applyRequest(() ->
                    drive.withVelocityX(-driver.getRawAxis(1) * 1) // Drive forward with negative Y (forward)
                        .withVelocityY(-driver.getRawAxis(0) * 1) // Drive left with negative X (left)
                        .withRotationalRate(-driver.getRawAxis(2) * MaxAngularRate) // Drive counterclockwise with negative X (left)
                )
            );
